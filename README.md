...
当前仓库包含可运行示例、基础单元测试和最小运行时装配，适合用于：
...
- `docs/`：设计、运维、产品说明与开源化整改方案

> 说明：仓库已完成 `core / standalone / demo` 第一轮拆分；后续仍会继续收敛 `standalone` 内部的自动配置、Web 与运行时边界，详见 `docs/open-source-standardization-plan.md`。
...
Demo 默认注册了 `EXAMPLE_PRODUCT` 示例业务类型，并预置了租户 `1` 与租户 `2` 的差异数据。也可以直接使用 `scripts/demo/` 下的脚本完成完整演示流程，详见 `docs/quickstart.md`。
...
- `docs/quickstart.md`：首次启动、请求示例与 Demo 主流程
- `docs/architecture.md`：当前模块边界与核心流程
- `docs/plugin-development.md`：插件扩展开发指南
- `docs/operations.md`：简明运行与排障入口
- `docs/versioning.md`：版本语义与兼容性约定
- `docs/releasing.md`：发布流程与检查清单
- `docs/open-source-standardization-plan.md`：开源化整改路线图
